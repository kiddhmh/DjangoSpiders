...
def searchweibo(request):
    ...
    if 'starttime' in request.GET:
        searchDic['starttime'] = request.GET['starttime']
    if 'endtime' in request.GET:
        searchDic['endtime'] = request.GET['endtime']
...
def main(serachDic):
    print('用户传输的参数是:', serachDic)
    username = serachDic['username']
    password = serachDic['password']
    login(username, password)
    while True:

        keyword = serachDic['keyword']
        if keyword == 'quit':
            sys.exit()
        startTime = serachDic['starttime']
        endTime = serachDic['endtime']
        savedir = 'CCICApp/weibo/data'
        interval = '50'
        ...
        while True:
            ...
            if endTime_stamp > endTime:
                break
            savedir = 'CCICApp/weibo/data/'+startTime_stamp+'_'+endTime_stamp
            instance = CollectData(keyword, startTime_stamp, endTime_stamp, savedir, interval)
            # 关键字写入文件
            with open(os.path.join('CCICApp/weibo/keyword.txt'), 'w', encoding='utf-8') as f:
                f.write(instance.keyword)
                f.close()
